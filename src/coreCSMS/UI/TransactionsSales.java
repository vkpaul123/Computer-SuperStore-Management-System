 ...
 * @author hp-pc
 */
public class TransactionsSales extends javax.swing.JInternalFrame {

    public NewMDIApplication mdiObj = null;
    
    /**
     * Creates new form TransactionsSales
     */
    public TransactionsSales(NewMDIApplication mdiObj) {
        this.mdiObj = mdiObj;
    ...
    public void initializeThisForm()    {
        try {
            ...
            ResultSet rs = st.executeQuery("select sales_id from Sales_Billing");
            if(rs.next())   {
                while(rs.next())    {}
                rs.previous();
                txtSalesID.setText(new Integer(rs.getInt(1) + 1).toString());
            }
            else
        ...
        txtSalesDate.setText(mdiObj.sendDate());
        txnOpenValue = false;
        ...
        fetchCustIDFromCustomersTable();
        refreshSalesOrder();
    }   
    
    public void fetchCustIDFromCustomersTable()   {
        try {
            Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
            Connection con = DriverManager.getConnection("jdbc:odbc:MyDSN_256");
    ...
    public void addExistingCustomerToTxn(String custID, String custName, String custPhNo)  {
        txtCustomerID.setText(custID);
        txtCustomerName.setText(custName);
        txtCustomerPhNo.setText(custPhNo);
    }
    
    public void addProductToSalesOrder(String productID, String productBrand, String productModel, String productPrice, String checkInputQty)    {
        try {
            Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
            Connection con = DriverManager.getConnection("jdbc:odbc:MyDSN_256");
            ...
            ResultSet rs = st.executeQuery("select * from Sales_Orders where sales_id=" + Integer.parseInt(txtSalesID.getText()) + " and product_id=" + Integer.parseInt(productID) + "");
            if(rs.next())   {
                JOptionPane.showMessageDialog(rootPane, "Product Already Exists in Products List (Sales Order)! Product Not Added!", "CSMS - Error" , JOptionPane.ERROR_MESSAGE);
            }
            else    {
                int salesQty=0;
                try {
                    salesQty = Integer.parseInt(JOptionPane.showInputDialog(rootPane, "Enter the Quantity to be sold for this Product...", "CSMS - Input", JOptionPane.INFORMATION_MESSAGE));
                }
                ...
                if(salesQty > 0)    {
                    if((Integer.parseInt(checkInputQty) - salesQty) >= 0)  {
                        try {
                            ...
                            int res = st1.executeUpdate("insert into Sales_Orders values(" + Integer.parseInt(txtSalesID.getText()) + ", " + Integer.parseInt(productID) + ", '" + productBrand + "', '" + productModel + "', " + Integer.parseInt(productPrice) + ", " + salesQty + ")");
                            JOptionPane.showMessageDialog(rootPane, "Product Added into Product List (Sales Order). " + res + " Row(s) added Successfully!", "CSMS - Message" , JOptionPane.INFORMATION_MESSAGE);
                            st1.close();
                            con1.close();
                        }
                        catch(ClassNotFoundException e) {}
                        ...
                        JOptionPane.showMessageDialog(rootPane, "Not Enough Stock for this product is available! Existing Quantity is: " + checkInputQty + ". Product Not Added!", "CSMS - Error" , JOptionPane.ERROR_MESSAGE);
                }
                else
                    JOptionPane.showMessageDialog(rootPane, "Product Not Added to Sales Transaction! Please Enter a Valid Number!", "CSMS - Error" , JOptionPane.ERROR_MESSAGE);
            }
            rs.close();
            st.close();
            con.close();
    ...
    public void refreshSalesOrder() {
        try {
            Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
            Connection con = DriverManager.getConnection("jdbc:odbc:MyDSN_256");
                    
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery("Select product_id, product_brand, product_model, sold_quantity, product_price from Sales_Orders where sales_id=" + Integer.parseInt(txtSalesID.getText()) + "");
            ResultSetMetaData rsMeta = rs.getMetaData();
            rs.setFetchDirection(ResultSet.FETCH_FORWARD);
            int columnCount = rsMeta.getColumnCount();
            DefaultTableModel tableModel = (DefaultTableModel) jTableSalesOrder.getModel();
    ...
    public void createNewTransaction()  {
        if(!txtCustomerName.getText().equals("") && !txtCustomerPhNo.getText().equals(""))  {
            if(jRadioButtonAddNew.isSelected()) {
                try {
                    ...
                    int res = st.executeUpdate("insert into Customers values(" + Integer.parseInt(txtCustomerID.getText()) + ", '" + txtCustomerName.getText() + "', '" + txtCustomerPhNo.getText() +"')");
                    JOptionPane.showMessageDialog(rootPane, "New Customer Added. " + res + " Row(s) added Successfully!", "CSMS - Message" , JOptionPane.INFORMATION_MESSAGE);
                    st.close();
                    con.close();
                }
                catch(ClassNotFoundException e) {}
            ...
            try {
                ...
                int res = st.executeUpdate("insert into Sales_Billing(sales_id, cust_id, sales_date) values(" + Integer.parseInt(txtSalesID.getText()) + ", " + Integer.parseInt(txtCustomerID.getText()) + ", '" + txtSalesDate.getText() +"')");
                JOptionPane.showMessageDialog(rootPane, "New Sales Transaction Added. " + res + " Row(s) added Successfully!", "CSMS - Message" , JOptionPane.INFORMATION_MESSAGE);
                st.close();
                con.close();
            }
            catch(ClassNotFoundException e) {}
    ...
    public void updateQuantitySalesOrders(String productID) {
        ...
        try {
            ...
            ResultSet rs4 = st4.executeQuery("Select sold_quantity from Sales_Orders where product_id=" + Integer.parseInt(productID) + " and sales_id=" + Integer.parseInt(txtSalesID.getText()) + "");
            if(rs4.next())  {
            ...
            if(salesQty > 0)    {
                if((Integer.parseInt(checkInputQty) - salesQty) >= 0)  {
                    ...
                    Statement st1 = con1.createStatement();
                    int res = st1.executeUpdate("update Sales_Orders set sold_quantity=" + salesQty + " where product_id =" + Integer.parseInt(productID) + " and sales_id=" + Integer.parseInt(txtSalesID.getText()) + "");
                    JOptionPane.showMessageDialog(rootPane, "Product Sales Quantity Updated in Product List (Sales Order). New Sales Quantity = " + salesQty + ". " + res + " Row(s) added Successfully!", "CSMS - Message" , JOptionPane.INFORMATION_MESSAGE);
                    st1.close();
                    con1.close();
    ...
    public void deleteProductSalesOrders(String productID)  {
        String checkInputQty, checkExistingQty;
        ...
        try {
            ...
            Statement st1 = con1.createStatement();
            int res = st1.executeUpdate("delete from Sales_Orders where product_id=" + Integer.parseInt(productID) + "");            
            st1.close();
            con1.close();
    ...
    public void deleteAllProductsFromSalesOrder()   {
        refreshSalesOrder();
        try {
            Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
            Connection con = DriverManager.getConnection("jdbc:odbc:MyDSN_256");
            ...
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery("Select product_id from Sales_Orders where sales_id=" + Integer.parseInt(txtSalesID.getText()) + "");
            while(rs.next())  {
                deleteProductSalesOrders(rs.getString(1));
            }
            rs.close();
            st.close();
            con.close();
    ...
    public void deleteSalesTransaction()    {
        try {
            ...
            Statement st = con.createStatement();
            int rs = st.executeUpdate("delete from Sales_Billing where sales_id=" + Integer.parseInt(txtSalesID.getText()) + "");
    ...
    public void calculateGrandTotal()   {
        ...
        try {
            ...
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery("Select product_price, sold_quantity from Sales_Orders where sales_id=" + Integer.parseInt(txtSalesID.getText()) + "");
            while(rs.next())    {
    ...
    public void completeSalesTransaction()  {
        calculateGrandTotal();
        ...
        if((Integer.parseInt(txtGrandTotal.getText())) > 0) {
            try {
                ...
                Statement st = con.createStatement();
                int res = st.executeUpdate("update Sales_Billing set sales_total_amount=" + Integer.parseInt(txtGrandTotal.getText()) + " where sales_id=" + Integer.parseInt(txtSalesID.getText()) + "");
                st.close();
                con.close();
            }
            catch(ClassNotFoundException e) {}
            ...
            JOptionPane.showMessageDialog(rootPane, "Transaction is Complete. Total Amount: " + (Integer.parseInt(txtGrandTotal.getText())) + ". Please Genereate Invoice for the Customer.", "CSMS - Message" , JOptionPane.INFORMATION_MESSAGE);
            ...
            JOptionPane.showMessageDialog(rootPane, "Products List (Sales Order) is Empty! Please add Some Products in the Products List (Sales Order)!", "CSMS - Error" , JOptionPane.ERROR_MESSAGE);
    }
    ...
    private void initComponents() {
        ...
        txtSalesDate = new javax.swing.JFormattedTextField();
        ...
        jTableSalesOrder = new javax.swing.JTable();
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Images/Icons/New Icons/IconsPNG/1458260998_cash_register.png"))); // NOI18N
        ...
        jLabel2.setText("SALES TRANSACTION");
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder("Sales Transaction"));
        ...
        txtSalesDate.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(new javax.swing.text.DateFormatter(new java.text.SimpleDateFormat("yyyy-MM-dd"))));
        ...
        jPanel3Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel3Layout.createSequentialGroup()
                ...
                .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(txtSalesDate, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.PREFERRED_SIZE, 120, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(txtSalesID, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.PREFERRED_SIZE, 120, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addContainerGap())
        );
        ...
        jPanel4.setBorder(javax.swing.BorderFactory.createTitledBorder("Customer Detials"));
        ...
        jRadioButtonAddNew.setText("Add New Customer");
        jRadioButtonAddNew.setToolTipText("Add New Customer");
        ...
        jRadioButtonAddExisting.setText("Add Existing Customer");
        jRadioButtonAddExisting.setToolTipText("Add Existing Customer");
        ...
        jPanel4Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel4Layout.createSequentialGroup()
                ...
                .addGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    ...
                    .addComponent(txtCustomerPhNo, javax.swing.GroupLayout.DEFAULT_SIZE, 230, Short.MAX_VALUE)
        ...
        jTableSalesOrder.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
        ...
        jTableSalesOrder.setFillsViewportHeight(true);
        jScrollPane1.setViewportView(jTableSalesOrder);
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        // TODO add your handling code here:
        if(jCheckBoxCreateNewTxn.isSelected() && !txtCustomerName.getText().equals("") && !txtCustomerPhNo.getText().equals(""))  {
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        // TODO add your handling code here:
        int selectedRowIndex = jTableSalesOrder.getSelectedRow();
        
        if(selectedRowIndex != -1)  {
            updateQuantitySalesOrders((jTableSalesOrder.getModel().getValueAt(selectedRowIndex, 0)).toString());
        }
        else       
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        ...
        if(selectedRowIndex != -1)  {
            deleteProductSalesOrders((jTableSalesOrder.getModel().getValueAt(selectedRowIndex, 0)).toString());
            JOptionPane.showMessageDialog(rootPane, "Product Removed from Products List (Sales Order). 1 Row(s) deleted Successfully!", "CSMS - Message" , JOptionPane.INFORMATION_MESSAGE);
        }
        else       
            JOptionPane.showMessageDialog(rootPane, "Please Select a Product first!", "CSMS - Error" , JOptionPane.ERROR_MESSAGE);
    ...
    private void jButton8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton8ActionPerformed
        // TODO add your handling code here:
        mdiObj.transactionSalesInvoiceObj.setInvoiceValues(txtSalesID.getText(), txtSalesDate.getText(), txtCustomerName.getText(), txtCustomerPhNo.getText(), txtGrandTotal.getText());
        mdiObj.transactionSalesInvoiceObj.setVisible(false);
        mdiObj.transactionSalesInvoiceObj.setVisible(true);
        mdiObj.transactionSalesInvoiceObj.setOriginalInvoice(true);
    ...
    private void jButton10ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton10ActionPerformed
        ...
        if(res == JOptionPane.YES_OPTION)   {
            deleteAllProductsFromSalesOrder();
    ...
    private javax.swing.JTable jTableSalesOrder;
    private javax.swing.JTextField txtCustomerID;
    private javax.swing.JTextField txtCustomerName;
    private javax.swing.JTextField txtCustomerPhNo;
    private javax.swing.JTextField txtGrandTotal;
    private javax.swing.JFormattedTextField txtSalesDate;
